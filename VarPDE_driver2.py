...
def parse_args():
	...
	parser.add_argument('--pnum_blocks', type=int, default=5, help='the number of residual blocks for pressure')
	parser.add_argument('--lam', type=float, default=0.001, help='the penalty parameters for Stokes equation')
	parser.add_argument('--batch_size', type=int, default=200, help='batch size')
	parser.add_argument('--num_iters', type=int, default=100000, help='the number of iterations')
	parser.add_argument('--lr_rate', type=float, default=1e-3, help='step size for optimization')
	parser.add_argument('--lr_decay', type=float, default=.9, help='decay rate after each 10000 iterations')
...
def main():
	...
	# construct model
	VarPDE = nnff.VarNets(spatial_range, pde_type, net_type, num_hidden, num_blocks, pnum_blocks, lam,  batch_size, num_iters, lr_rate, lr_decay, output_path)
	# train model 
	VarPDE.train()
...
#		gridv_path = '/home/tkj/rap_prog/pyfile/t3f_UQ/data/stokes_gravity_grid.mat'
#		gridp_path = '/home/tkj/rap_prog/pyfile/t3f_UQ/data/stokes_gravity_pressuregrid.mat'
		#fem_sol = sio.loadmat(femsol_path)['u']
		gridv_path = '/home/tangkj/varpde/data_varpde/stokes_gravity_grid.mat'
		gridp_path = '/home/tangkj/varpde/data_varpde/stokes_gravity_pressuregrid.mat'
		...
		grid = (gridv, gridp)
		dnn_sol, problem_type = VarPDE.test(grid)
		print('-----dnn solution-----', dnn_sol)
		sio.savemat('dnn_stokessol_gravity2.mat', {'stokes_sol': dnn_sol})
		print('----- sava data (stokes_gravity2) done-----')
		print('----- PDE type------', problem_type)
	else:
